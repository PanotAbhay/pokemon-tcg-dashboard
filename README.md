...
The dataset holds card *attributes* only (name, set, series, rarity, type, HP, artist, release date) — no pricing data. The product displays a searchable/filterable card catalog, a headline stat strip (total cards, sets, series, artists, avg HP), and three trend charts: card count by rarity, card count by energy type, and cards printed per year.
...
| Stats | `GET /api/stats/overview`, `by-rarity`, `by-type`, `by-set`, `by-year` | Aggregated counts/averages powering the stat strip and charts |
...
Open **http://127.0.0.1:5000** in a browser — Flask serves the client and API together, so no second server is needed.

**Filters:** use the search box and dropdowns (set, series, rarity, type, supertype, year range) in the browser UI; they combine and re-query `/api/cards` automatically. Clear all with the "clear filters" action shown in the empty state.
